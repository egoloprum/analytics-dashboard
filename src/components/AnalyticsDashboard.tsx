...
const Badge = ({percentage}: {percentage: number}) => {
   ...
   const neutralClassname = 'bg-zing-900/25 text-zing-400 ring-zing-400/25'
   const negativeClassname = 'bg-red-900/25 text-red-400 ring-red-400/25'

   return (
      <span className={`inline-flex gap-1 items-center rounded-md px-2 py-1 text-xs font-medium ring-1 ring-inset ${isPositive ? positiveClassname : isNeutral ? neutralClassname : negativeClassname}`}>{isPositive ? <ArrowUpRight className="h3 w-3" /> : null}
      {isNeutral ? <ArrowRight className="h3 w-3" /> : null}
      {isNegative ? <ArrowDownRight className="h3 w-3" /> : null}
